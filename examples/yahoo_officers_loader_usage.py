...
async def demo_single_symbol_officers() -> None:
    ...
    if officers:
        ...
        sorted_officers = sorted(officers, key=lambda x: x.total_pay or 0, reverse=True)
        ...
        for i, officer in enumerate(sorted_officers[:5], 1):
            ...
            print(
                f"     Exercised Value: ${officer.exercised_value:,}"
                if officer.exercised_value
                else "     Exercised Value: N/A"
            )
            print(
                f"     Unexercised Value: ${officer.unexercised_value:,}"
                if officer.unexercised_value
                else "     Unexercised Value: N/A"
            )
            print()

        # Load to database
        print("Loading officers data to database...")
    ...
    for symbol in symbols:
        ...
        try:
            ...
            if officers:
                ...
                pay_str = (
                    f"${top_officer.total_pay:,}" if top_officer.total_pay else "N/A"
                )
                print(f"  [OK] {len(officers)} officers loaded")
                print(
                    f"  Top paid: {top_officer.name} ({top_officer.title}) - {pay_str}"
                )
            else:
                print("  [WARN] No officers data available")
...
async def demo_officers_analysis() -> None:
    ...
    officers_with_pay = [
        (symbol, officer) for symbol, officer in all_officers if officer.total_pay
    ]

    if officers_with_pay:
        # Sort by compensation
        sorted_by_pay = sorted(
            officers_with_pay, key=lambda x: x[1].total_pay, reverse=True
        )
        ...
        for i, (symbol, officer) in enumerate(sorted_by_pay[:10], 1):
            print(
                f"  {i:2d}. {officer.name[:30]:30s} | {symbol:6s} | ${officer.total_pay:>12,}"
            )
    ...
    print("Most common executive titles:")
    ...
    print("\n--- AGE ANALYSIS ---")
    officers_with_age = [
        (symbol, officer) for symbol, officer in all_officers if officer.age
    ]
    ...
    if officers_with_age:
        ...
        print(
            f"Age Statistics (based on {len(officers_with_age)} officers with age data):"
        )

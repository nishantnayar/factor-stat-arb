...
def check_venv() -> Check:
    ...
    return Check(
        "virtualenv", ok, sys.prefix if ok else f"{sys.prefix} (expected {expected})"
    )
...
def check_database() -> Check:
    ...
    try:
        ...
        return Check(
            "database", ok, f"connected to {db}, {found}/{len(schemas)} core schemas"
        )
...
def check_alpaca() -> Check:
    ...
    if "paper" not in s.alpaca_base_url:
        return Check(
            "alpaca (paper)", False, f"base_url not paper: {s.alpaca_base_url}"
        )
    if not (s.alpaca_api_key and s.alpaca_secret_key):
        return Check(
            "alpaca (paper)",
            True,
            "paper endpoint, KEYS MISSING (set to trade)",
            critical=False,
        )
    ...
    try:
        ...
        return Check(
            "alpaca (paper)",
            False,
            f"auth failed: {type(e).__name__}: {e}",
            critical=False,
        )
...
def _start_prefect(env, s) -> "subprocess.Popen | None":  # noqa: F821
    ...
    ensure_profile()
    p = subprocess.Popen(
        ["prefect", "server", "start"], env=env, stdin=subprocess.DEVNULL
    )
    for _ in range(40):
        if _prefect_healthy(api):
...
def start_services(services: list[str]) -> int:
    ...
    if unknown:
        print(
            f"Unknown service(s): {', '.join(unknown)}. Known: {', '.join(ALL_SERVICES)}"
        )
        return 2

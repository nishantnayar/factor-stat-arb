...
def test_prefect_imports() -> bool:
    ...
    try:
        from prefect import flow, get_client, task  # noqa: F401

        # Test Prefect 3.x imports
        try:
            from prefect.server.schemas.schedules import CronSchedule  # noqa: F401
        except ImportError:
            # Prefect 3.x may have different import path
            from prefect.schedules import CronSchedule  # noqa: F401
...
def check_prefect_server() -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info(
                "Prefect 3.4.14 server is running at http://localhost:4200 [OK]"
            )
...
Examples:
  ...
  python scripts/test_prefect.py
    ...
    parser.add_argument("--retry", action="store_true", help="Test retry logic")
    ...
    if not server_available:
        ...
        logger.info(
            "   If connection fails, try: set PREFECT_API_URL=http://localhost:4200/api"
        )
    ...
    try:
        ...
        logger.info("\nTroubleshooting:")
        logger.info("   1. Verify server is running: prefect server start")
        logger.info("   2. Check API URL is set: echo $env:PREFECT_API_URL")
        logger.info(
            "   3. Set explicitly: $env:PREFECT_API_URL='http://localhost:4200/api'"
        )
        return 1
